...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask_ratio: float = 0.0):
        ...
        if mask_ratio > 0.0:
            ...
            x = x[batch_indices, keep_indices]  # masked x: [B, num_keep, C]

            # 2. positional embedding도 같이 선택
            pos_embed = self.positional_embedding[1:]  # exclude CLS
            pos_embed = pos_embed[keep_indices]  # [B, num_keep, C]
        else:
            pos_embed = self.positional_embedding[1:].unsqueeze(0).expand(B, -1, -1)

        # 3. CLS token 붙이기
        cls_token = self.class_embedding.to(x.dtype).unsqueeze(0).unsqueeze(0).expand(B, 1, C)
        x = torch.cat([cls_token, x], dim=1)

        pos_embed_cls = self.positional_embedding[0].unsqueeze(0).unsqueeze(0).expand(B, 1, C)
        pos_embed = torch.cat([pos_embed_cls.to(x.dtype), pos_embed.to(x.dtype)], dim=1)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        return self.visual(image.type(self.dtype), mask_ratio=self.mask_ratio)
...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, mask_ratio: float, jit: bool = False, download_root: str = None):
    """Load a CLIP model
